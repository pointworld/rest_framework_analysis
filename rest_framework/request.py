...
class Request(object):
    ...
    def user(self):
        ...
        ## 返回与当前 request 关联的用户，会被当前请求提供的认证类所认证
        """
    ...
    def _load_data_and_files(self):
        ...
        ## 将请求内容解析到 self.data 中（request.data）
        """
    ...
    def _authenticate(self):
        ...
        for authenticator in self.authenticators:
            try:
                ## 执行认证类的 authenticate 方法
                ## 1. 如果该方法抛出异常，self._not_authenticated() 会被执行
                ## 2. 如果没有抛出异常，有返回值（None 或元组（request.user, request.auth））
                ##    如果返回值为 None，表示当前认证不处理，交由下一个认证来处理
                user_auth_tuple = authenticator.authenticate(self)
            except exceptions.APIException:
                self._not_authenticated()
                ## 执行 _not_authenticated() 方法后，继续将异常抛出
                raise

            if user_auth_tuple is not None:
                self._authenticator = authenticator
                ## 解包认证后的结果，赋值给 self.user, self.auth
                self.user, self.auth = user_auth_tuple
                return

        ## 如果所有的认证都返回 None，将执行该方法
        self._not_authenticated()
